...
def initialize_node():
    ...
    rospy.init_node('ultrasonic_pub_5')
    ...
    GPIO.setup(GPIO_SIG_5, GPIO.OUT)
    GPIO.output(GPIO_SIG_5, 0)
    time.sleep(0.000002)
    #send trigger signal
    GPIO.output(GPIO_SIG_5, 1)
    time.sleep(0.000005)
    GPIO.output(GPIO_SIG_5, 0)
    GPIO.setup(GPIO_SIG_5, GPIO.IN)
    while GPIO.input(GPIO_SIG_5) == 0:
        starttime = time.time()
    while GPIO.input(GPIO_SIG_5) == 1:
        endtime = time.time()
    duration5 = endtime - starttime

    while True:
       
        distance5 = (duration5*34000)/2.0
